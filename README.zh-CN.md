...
BBBoard 是 macOS 原生的常驻任务看板：以紧凑挂件形态挂在桌面上，悬停展开，用 Apple 设备端 Foundation Models 把混乱的脑内想法变成结构化任务。没有要打开的窗口，没有要切换的 App。

## 功能

- **桌面挂件** — 位于桌面图标之上、普通窗口之下（与 Apple 桌面小组件同层）。悬停展开，头部拖动换位，位置持久化。
- **横向看板** — TODAY | DOING | WAITING 三列，BACKLOG / DONE 为折叠区。卡片拖拽改状态。
- **Brain Dump** — 粘贴混乱想法（"明天跟进 PRG 的 API key，等 Peter 回复"），AI 拆成结构化提案（标题/状态/日期/等待对象），逐条确认入库。支持中英混输。
- **降级是一等功能** — 没有 Apple Intelligence 时，规则降级解析器（NSDataDetector + 中文日期规则 + 等待语识别）产出同构提案；手动录入永远可用。
...
- Apple Intelligence 开启可获得 AI 解析（可选——降级解析器兜底）
...
> 注意：命令行构建需要完整 Xcode 工具链（`export DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer`）——Command Line Tools 缺 FoundationModels 宏插件和 XCTest。

## 架构

```
BoardApp        macOS App：桌面挂件（AppKit NSPanel）、Peek 面板、全局快捷键、SwiftUI 视图
  ├─ AIParser   Foundation Models 封装（@Generable）、可用性门控、规则降级解析器、日期解析
  ├─ RuleEngine 纯函数：截止/等待/停滞规则 + Focus 打分，全单测
  └─ TaskStore  GRDB/SQLite：迁移、CRUD、activity_log、Focus 快照、corrections
```

设计原则：

- **LLM 只做语义理解** — 拆任务、分类、判重。日期计算、超时判断、排序打分一律走规则代码。
- **AI 产出永远是提案** — 不经用户确认不入库；每次修改记入 `corrections` 表。
- **所有写操作记日志** — `activity_log`（created/edited/status_changed/completed/deleted）是 Daily Focus 和智能清理质量的上限。

## 测试

```bash
swift test    # 80 个测试：规则引擎、日期解析、降级解析器、存储层
```
